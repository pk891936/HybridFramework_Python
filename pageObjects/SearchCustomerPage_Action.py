...
class SearchCustomer(driver_mapping):
    ...
    def __init__(self, driver_mapping):
        super(SearchCustomer, self).__init__(driver_mapping)
    ...
    def getNoOfRows(self):
        return len(self.driver.getelements(By.XPATH, self.tableRows_xpath))

    def getNoOfColumns(self):
        return len(self.driver.getelements(By.XPATH, self.tableColumns_xpath))

    def searchCustomerByEmail(self,email):
        flag=False
        for r in range(1,self.getNoOfRows()+1):
          table=self.driver.getelement(By.XPATH, self.table_xpath)
